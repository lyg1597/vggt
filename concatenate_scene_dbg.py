...
def plot_3d_points(points: np.ndarray, color: str = 'red', plotter: pv.Plotter = None, point_size=10) -> pv.Plotter:
    """
    Plots 3D points on a PyVista plotter instance.

    This function takes a NumPy array of 3D coordinates, a color, and an
    optional PyVista plotter. It adds the points to the plotter and returns
    the updated plotter instance.

    Args:
        points (np.ndarray): A NumPy array with shape (N, 3), where N is the
                             number of points. Each row represents a point's
                             (x, y, z) coordinates.
        color (str, optional): The color for the points. Can be a string name
                               like 'red', 'blue', etc., or a hex code.
                               Defaults to 'red'.
        plotter (pv.Plotter, optional): An existing PyVista plotter object.
                                        If None, a new plotter will be created.
                                        Defaults to None.

    Returns:
        pv.Plotter: The plotter object with the new points added.
    """
    # --- Input Validation ---
    if not isinstance(points, np.ndarray) or points.ndim != 2 or points.shape[1] != 3:
        raise ValueError("Input 'points' must be a NumPy array with shape (N, 3).")

    # --- Plotter Initialization ---
    # If no plotter is provided, create a new one. This makes the function
    # flexible for both standalone use and for adding to existing scenes.
    if plotter is None:
        plotter = pv.Plotter()
        print("No plotter provided. A new one has been created.")

    # --- Plotting ---
    # The add_points function is the core of PyVista's point plotting.
    # We specify a 'point_size' to make the points more visible.
    plotter.add_points(
        points,
        color=color,
        point_size=point_size,
        render_points_as_spheres=True # Makes points look nicer
    )
...
def concate_results(result1, result2=None, overlap=[]):
    ...
    overlap2_transformed = a_to_b(overlap2, T, s)
    ...
    return extrinsics1, extrinsics2_transformed, overlap1, overlap2_transformed 

def main(args):
    # model, device = load_vggt_model()
    images = preprocess_input_images(args.image_dir, torch.device('cuda'))
    if images is None:
        return
    images1 = images[:15]
    images2 = images[1:]
    ...
    # )
    final_results1 = np.load('tmp1.npz')
    ...
    # )
    final_results2 = np.load('tmp2.npz')
    
    extrinsics1, extrinsics2_transformed, overlap1, overlap2_transformed  = concate_results(final_results1, final_results2, list(range(1,15)))
    fig = pv.Plotter()
    fig = plot_3d_points(extrinsics1[:,:3,3], 'red', fig)
    fig = plot_3d_points(extrinsics2_transformed[:,:3,3], 'green', fig)
    fig = plot_3d_points(overlap1[:,:3,3], 'blue', fig, point_size=15)
    fig = plot_3d_points(overlap2_transformed[:,:3,3], 'orange', fig, point_size=15)
    fig.show()
